...
bool sa818_set_cdcss_tx(uint16_t code, bool inverted = false);
bool sa818_set_cdcss_rx(uint16_t code, bool inverted = false);
...
struct SA818GroupConfig {
    ...
    const char *tx_subaudio;  // e.g. "0000", "0008", "023N", "023I"
    const char *rx_subaudio;
    SA818Squelch squelch;
    bool wide_band;
};

struct SA818GroupState {
    uint32_t tx_freq_khz;
    uint32_t rx_freq_khz;
    char tx_subaudio[8];
    char rx_subaudio[8];
